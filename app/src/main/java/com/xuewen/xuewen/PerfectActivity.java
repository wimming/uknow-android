...
public class PerfectActivity extends AppCompatActivity {
    ...
    private void patchModifyUserInfoService(int uid) {
        ...
        if (!Validator.validateNoEmpty(
            ...
            Toast.makeText(PerfectActivity.this, ToastMsg.INVALID_ARG_EMPTY, Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        call.enqueue(new Callback<UUidPResult>() {
            ...
            public void onResponse(Call<UUidPResult> call, Response<UUidPResult> response) {
                if (!response.isSuccessful()) {
                    Toast.makeText(PerfectActivity.this, ToastMsg.SERVER_ERROR, Toast.LENGTH_SHORT).show();
                    return;
                }
                if (response.body().status != 200) {
                    Toast.makeText(PerfectActivity.this, response.body().errmsg, Toast.LENGTH_SHORT).show();
                    return;
                }
